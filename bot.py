    ...
    def send_otp():
        try:
            ...
            if "API_ID" in error_msg or "API_HASH" in error_msg:
                ...
                error_msg = "❌ Phone number invalid hai. Sahi format: +919876543210"
...
def handle_otp(m):
    ...
    # OTP expiry check - 5 minute
    otp_age = time.time() - client_data.get("timestamp", 0)
    if otp_age > 300:
        bot.reply_to(m, "❌ <b>OTP 5 minute se zyada purana hai!</b>\n\n🔄 <b>Resend OTP</b> button dabao.", reply_markup=get_otp_keyboard())
        return
...
if __name__ == "__main__":
    ...
    # Flask web server chalao (Render ko port chahiye)
    port = int(os.environ.get("PORT", 10000))
    app.run(host="0.0.0.0", port=port)def cmd_help(message):
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "⛔ <b>Unauthorized.</b>")
        return
